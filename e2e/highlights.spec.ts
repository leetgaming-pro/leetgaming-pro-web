...
test.describe("Highlights Page", () => {
  test.describe("Page Loading", () => {
    ...
    test("should display breadcrumbs navigation", async ({ page }) => {
      ...
      const hasHighlights = await highlightsBreadcrumb
        .first()
        .isVisible()
        .catch(() => false);
  ...
  test.describe("Event Types", () => {
    test("should display headshot events", async ({ page }) => {
      ...
      const hasHeadshot = await headshotText
        .first()
        .isVisible()
        .catch(() => false);
    ...
    test("should display clutch events", async ({ page }) => {
      ...
      const hasClutch = await clutchText
        .first()
        .isVisible()
        .catch(() => false);
  ...
  test.describe("Empty State", () => {
    test("should show empty state when no events", async ({ page }) => {
      ...
      const emptyState = page.getByText(
        /no highlights|no events|nothing here/i,
      );
  ...
  test.describe("Professional Esports Branding", () => {
    test("should display professional esports branding", async ({ page }) => {
      ...
      expect(hasGradients || hasHeadings).toBe(true);

      // Check for professional typography - any heading with highlights text
      const professionalText = page
        .locator('h1, h2, [class*="font-bold"]')
        .filter({ hasText: /highlights/i });
      const hasProfessionalText = (await professionalText.count()) > 0;
      expect(hasProfessionalText || hasNavigation).toBe(true);
    });

    test("should display public highlights according to resource ownership", async ({
      page,
    }) => {
      await page.goto("/highlights", { waitUntil: "domcontentloaded" });
      await page.waitForLoadState("domcontentloaded");
      ...
      // Check that highlight cards are displayed (public data)
      const highlightCards = page.locator(
        '[data-testid="highlight-card"], .card, article',
      );
      ...
      if (cardCount > 0) {
        ...
        // Check for event type indicators (public highlight data)
        const eventBadges = firstCard.locator(
          '[data-testid*="event"], .badge, .chip',
        );
        const hasEventInfo = (await eventBadges.count()) > 0;
        expect(hasEventInfo).toBe(true);
      }
    });

    test("should handle infinite scroll with professional loading states", async ({
      page,
    }) => {
      await page.goto("/highlights", { waitUntil: "domcontentloaded" });
      await page.waitForLoadState("domcontentloaded");
      ...
      const loadingElements = page.locator(
        '[data-testid="loading"], .spinner, .skeleton',
      );
