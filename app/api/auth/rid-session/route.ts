...
// Check if we're in a local development environment (K8s local or dev server)
// SECURITY: In production, secure flag is ALWAYS true regardless of env var content
const isLocalDev =
...
export async function GET(_request: NextRequest) {
  try {
    ...
    // Return session info (exclude actual token and sensitive IDs for security)
    return NextResponse.json({
      authenticated: true,
      metadata: {
        expiresAt: metadata.expiresAt,
        intendedAudience: metadata.intendedAudience,
        // Only expose non-sensitive metadata to the client
        hasResourceOwner: !!metadata.resourceOwner,
      },
    });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!tokenId || !resourceOwner) {
      console.warn("[RID-Session POST] Missing required fields");
      return NextResponse.json(
