...
const mapAPITournamentToDisplay = (t: APItournament): TournamentDisplay => ({
  ...
  type:
    (t.format?.replace("_", "-") as TournamentDisplay["type"]) ||
    "single-elimination",
...
export default function TournamentsPage() {
  ...
  const [selectedTournament, setSelectedTournament] =
    useState<TournamentDisplay | null>(null);
  const [registering, setRegistering] = useState(false);

  // Fetch tournaments from API using SDK context
  useEffect(() => {
    async function fetchTournaments() {
      ...
      try {
        ...
        const mappedTournaments: TournamentDisplay[] = (
          result.tournaments || []
        ).map(mapAPITournamentToDisplay);
  ...
  const handleConfirmRegistration = async () => {
    if (!selectedTournament || !isAuthenticated || !user || !isReady) return;

    setRegistering(true);
    try {
      const result = await sdk.tournaments.registerPlayer(
    ...
    <div className="w-full max-w-7xl xl:max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-12 xl:px-16 2xl:px-24 py-8 lg:py-12 xl:py-16">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 lg:gap-6 mb-8 lg:mb-12">
        <div className="flex items-center gap-4 lg:gap-6">
          <div
            className="w-12 h-12 lg:w-16 lg:h-16 flex items-center justify-center bg-gradient-to-br from-[#FF4654] to-[#FFC700] dark:from-[#DCFF37] dark:to-[#34445C] rounded-none"
            style={{
              clipPath:
          ...
          >
            <Icon
              icon="solar:cup-star-bold"
              width={28}
              className="text-[#F5F0E1] dark:text-[#1a1a1a] lg:w-8 lg:h-8"
            />
          </div>
          <div>
            <h1 className="text-2xl lg:text-3xl xl:text-4xl font-bold text-[#34445C] dark:text-[#F5F0E1]">
              Tournaments
            </h1>
            <p className="text-sm lg:text-base text-[#34445C]/60 dark:text-[#F5F0E1]/60">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-8 lg:mb-12">
          <Card className="rounded-none border border-[#FF4654]/20 dark:border-[#DCFF37]/20">
            <CardBody className="text-center py-6">
              <div
                className="w-12 h-12 mx-auto mb-2 flex items-center justify-center bg-gradient-to-br from-[#FF4654] to-[#FFC700] dark:from-[#DCFF37] dark:to-[#34445C]"
                style={{
                  clipPath:
                    "polygon(0 0, 100% 0, 100% calc(100% - 8px), calc(100% - 8px) 100%, 0 100%)",
                }}
      ...
      <Card className="mt-8 bg-gradient-to-r from-[#FF4654] to-[#FFC700] dark:from-[#34445C] dark:to-[#1e2a38] rounded-none overflow-hidden relative">
        ...
        <CardBody className="text-center py-8">
          <div
            className="w-16 h-16 mx-auto mb-4 flex items-center justify-center bg-[#F5F0E1]/20 dark:bg-[#DCFF37]/20"
            style={{
              clipPath:
                "polygon(0 0, 100% 0, 100% calc(100% - 12px), calc(100% - 12px) 100%, 0 100%)",
            }}
          ...
          <p className="text-[#F5F0E1]/80 mb-6">
            Create and manage tournaments for your community with our
            easy-to-use tools
          </p>
                ...
                {selectedTournament && (
                  <div className="space-y-4">
                    ...
                    <p className="text-sm text-default-600">
                      By registering, you agree to the tournament rules and code
                      of conduct.
                    </p>
                  </div>
                )}
