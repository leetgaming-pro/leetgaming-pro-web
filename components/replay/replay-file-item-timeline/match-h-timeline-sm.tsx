...
export default function MatchTimelineHorizontalSmall(
  props: MatchTimelineProps
) {
  const { rounds } = props;
  ...
  return (
    <div
      className={`relative w-full overflow-x-auto ${electrolize.className}`}
      {...props}
    >
        ...
        <div className="flex w-full">
          {rounds.map((round, index: number) => (
            <React.Fragment key={index}>
              ...
              <div
                className={`flex flex-col w-6 mx-0.1rem pr-[0.1rem] ${electrolize.className}`}
              >
                {getConditionalCard(round, "ct")}
                {getConditionalCard(round, "t")}
              </div>
              <div
                className={`absolute pt-4 mx-0.1rem pr-[0.1rem] pl-${
                  index + 1 >= 10 ? "0" : "1"
                }`}
              >
                {
