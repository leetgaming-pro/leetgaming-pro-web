...
export const getColor = (round: { winner: string | null }) => {
  if (round.winner === "ct") {
...
export default function TimelineWinnerCard({
  roundData,
}: TimelineWinnerCardProps) {
  const radiusTarget = roundData.winner === "ct" ? "Top" : "Bottom"; // TODO: handle shifting sides + separators
  const gradientString =
    roundData.winner === "ct"
      ? "linear-gradient(to top, rgba(155, 210, 255, 0.33), transparent)"
      : "linear-gradient(to bottom, rgba(255, 210, 155, 0.33), transparent)";
