...
function printHeader(): void {
  ...
  console.log(
    colorize("║   ", "cyan") +
      colorize("LeetGaming.PRO", "bright") +
      colorize(" - Interactive Environment Setup          ║", "cyan")
  );
  console.log(
    colorize(
      "║                                                              ║",
...
async function askWithDefault(
  ...
  const answer = await prompt(
    `${question} ${colorize(`[${defaultValue}]`, "dim")}: `
  );
...
async function main(): Promise<void> {
  ...
  if (setupSteam) {
    ...
    if (useGeneratedVhash) {
      ...
      config.STEAM_VHASH_SOURCE = await askRequired(
        "Enter your STEAM_VHASH_SOURCE"
      );
    }
  } else {
  ...
  if (setupStripe) {
    ...
    config.STRIPE_WEBHOOK_SECRET =
      (await askOptional("Stripe Webhook Secret (whsec_...)")) ||
    ...
    config.NEXT_PUBLIC_STRIPE_PRO_PRICE_ID =
      (await askOptional("Pro subscription Price ID")) ||
      "price_your_pro_price_id";
    config.NEXT_PUBLIC_STRIPE_TEAM_PRICE_ID =
      (await askOptional("Team subscription Price ID")) ||
  ...
  config.NEXT_PUBLIC_ENABLE_WALLET = (
    await askYesNo("Enable Wallet?", true)
  ).toString();
  config.NEXT_PUBLIC_ENABLE_PRIZE_POOLS = (
    await askYesNo("Enable Prize Pools?", true)
  ).toString();
  config.NEXT_PUBLIC_ENABLE_TOURNAMENTS = (
    await askYesNo("Enable Tournaments?", true)
  ).toString();
  config.NEXT_PUBLIC_ENABLE_AI_MATCHMAKING = (
    await askYesNo("Enable AI Matchmaking?", true)
  ).toString();
  config.NEXT_PUBLIC_ENABLE_CRYPTO = (
    await askYesNo("Enable Crypto Payments?", false)
  ).toString();
  config.NEXT_PUBLIC_ENABLE_PAYPAL = (
    await askYesNo("Enable PayPal?", false)
  ).toString();
  ...
  config.LOG_LEVEL = await askWithDefault(
...
`;
  ...
  console.log(
    "1. Review your .env.local file and update any placeholder values"
  );
  console.log("2. Ensure your backend (replay-api) uses matching secrets:");
  console.log(
    `   - NEXTAUTH_SECRET: ${config.NEXTAUTH_SECRET?.substring(0, 20)}...`
  );
  if (config.STEAM_VHASH_SOURCE) {
    console.log(
      `   - STEAM_VHASH_SOURCE: ${config.STEAM_VHASH_SOURCE.substring(
        0,
        10
      )}...`
    );
